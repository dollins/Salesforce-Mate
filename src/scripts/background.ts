...
chrome.runtime.onMessage.addListener((request: Message, sender, sendResponse) => {
    if (request.message === "getSfHost") {
        ...
        //cookieStoreId is not part of default definition of chrome type.
        const cookieStoreId = (sender.tab as chrome.tabs.Tab & { cookieStoreId?: string }).cookieStoreId;

        chrome.cookies.get({ url: url.href, name: "sid", storeId: cookieStoreId }, (cookie) => {
            if (!cookie) {
                sendResponse(null);
                return;
            }
            ...
            chrome.cookies.getAll(
                { name: "sid", domain: "salesforce.mil", secure: true, storeId: cookieStoreId },
                (cookies) => {
                    ...
                    if (sessionCookie) {
                        ...
                        chrome.cookies.getAll(
                            { name: "sid", domain: "salesforce.com", secure: true, storeId: cookieStoreId },
                            (cookies) => {
                                ...
                                if (sessionCookie) {
                                    ...
                                    chrome.cookies.getAll(
                                        { name: "sid", domain: "cloudforce.mil", secure: true, storeId: cookieStoreId },
                                        (cookies) => {
                                            sessionCookie = cookies.find((c) => c.value.startsWith(orgId + "!"));
                                            if (sessionCookie) {
                                                sendResponse(sessionCookie.domain);
    ...
    if (request.message === "getSession") {
        const cookieStoreId = (sender.tab as chrome.tabs.Tab & { cookieStoreId?: string }).cookieStoreId;
        chrome.cookies.get(
            { url: "https://" + request.sfHost, name: "sid", storeId: cookieStoreId },
            (sessionCookie) => {
                if (!sessionCookie) {
                    sendResponse(null);
                    return;
...
function stringToUrl(input: string): URL | null {
    try {
